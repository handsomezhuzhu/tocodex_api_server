...
// request, none of which the real ToCodex VSCode extension sends on its
// `sdk` profile.  For the CTF impersonation requirements we need byte-level
// control of the wire headers, so every outbound request goes through this
// helper instead.
//
...
//
// Header ordering: pass a HeaderList (from lib/headers.js) or an ordered
// array-of-pairs [[name, value], ...].  Plain objects are accepted for
// back-compat but lose deterministic order.  node:http writes flat form
// `['k', 'v', 'k', 'v', ...]` to the wire in-order, so we normalise
// everything to that.
...
// Normalise whatever the caller passed as `headers` into the flat
// [name,value,name,value,...] form that node:http accepts and preserves
// verbatim on the wire.
function toFlatHeaders(headers) {
  if (headers == null) return [];
  if (typeof headers.toFlat === "function") return headers.toFlat();
  if (Array.isArray(headers)) {
    // Accept [[k,v], ...] or already-flat ['k','v','k','v', ...].
    if (headers.length === 0) return [];
    if (Array.isArray(headers[0])) {
      const out = [];
      for (const pair of headers) {
        if (!pair || pair.length < 2 || pair[1] == null) continue;
  ...
  // Object form — preserve Object.keys order (not guaranteed byte-exact
  // but it's what callers used before HeaderList; still better than the
  // lexical re-sort some HTTP libraries do).
  const out = [];
