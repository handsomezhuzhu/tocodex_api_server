...
function openaiResponseToResponses(payload, requestedModel) {
  ...
  const promptTokens = (payload.usage && payload.usage.prompt_tokens) || 0;
  const completionTokens = (payload.usage && payload.usage.completion_tokens) || 0;
  return {
    id: responseId,
    object: "response",
    ...
    usage: {
      ...
      // `total_tokens` is REQUIRED by the Responses API schema — Codex CLI
      // (Rust `async-openai`) deserialises strictly and aborts the stream
      // with `missing field 'total_tokens'` when absent.  See issue #1.
      total_tokens:
        (payload.usage && payload.usage.total_tokens) ||
...
async function* responsesStreamFromOpenAI(openaiPayloadIter, requestedModel, responseId) {
  ...
  try {
    ...
    const streamPromptTokens = Number(usage.prompt_tokens || 0);
    const streamCompletionTokens = Number(usage.completion_tokens || 0);
    yield formatOpenAISSE({
      ...
      response: {
        ...
        usage: {
          ...
          // `total_tokens` is REQUIRED here — missing it crashes Codex
          // CLI's Rust deserialiser mid-stream (issue #1).
          total_tokens:
